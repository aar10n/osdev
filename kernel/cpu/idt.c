...
extern uintptr_t isr_stubs;
struct idt_gate shared_idt[IDT_GATES];

void idt_percpu_init() {
  struct idt_gate *idt = shared_idt;
  if (curcpu_is_boot) {
    // first time - initialize the IDT with the asm stubs
    uintptr_t asm_handler = (uintptr_t) &isr_stubs;
    for (int i = 0; i < IDT_GATES; i++) {
      shared_idt[i] = gate(asm_handler, KCODE_SEG, 0, INTERRUPT_GATE, 0, 1);
