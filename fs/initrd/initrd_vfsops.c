...
static int initrd_vfs_mount_v1(vfs_t *vfs, device_t *device, ventry_t *mount_ve, __move ventry_t **root);
static int initrd_vfs_mount_v2(vfs_t *vfs, device_t *device, ventry_t *mount_ve, __move ventry_t **root);
...
int initrd_vfs_mount(vfs_t *vfs, device_t *device, ventry_t *mount_ve, __move ventry_t **root) {
  ...
  // read just the signature first to determine version
  if ((res = d_read_n(device, 0, signature, 6)) < 0 || res != 6) {
    EPRINTF("mount: failed to read signature\n");
    if (res >= 0)
      res = -EIO;
    return (int) res;
  }

  // determine version and dispatch
  if (strncmp(signature, "INITv1", 6) == 0) {
    return initrd_vfs_mount_v1(vfs, device, mount_ve, root);
  } else if (strncmp(signature, "INITv2", 6) == 0) {
    return initrd_vfs_mount_v2(vfs, device, mount_ve, root);
  } else {
...
static int initrd_vfs_mount_v1(vfs_t *vfs, device_t *device, ventry_t *mount_ve, __move ventry_t **root) {
  ...
  for (int i = 0; i < header.entry_count; i++) {
    ...
    if (entry->entry_type == 'f') {
      ...
      rd_node->data_size = entry->data_size;
      rd_node->checksum = 0;  // v1 has no checksums
      node->data = rd_node;
...
static int initrd_vfs_mount_v2(vfs_t *vfs, device_t *device, ventry_t *mount_ve, __move ventry_t **root) {
  kio_t tmp;
  ssize_t res;
  initrd_header_v2_t header;
  if ((res = d_read_n(device, 0, &header, sizeof(header))) < 0 || res != sizeof(header)) {
    EPRINTF("mount: failed to read v2 header\n");
    if (res >= 0)
      res = -EIO;
    return (int) res;
  }
  ...
  if (header.flags != 0) {
    EPRINTF("mount: no flags are supported yet\n");
    return -ENOTSUP;
  }
  ...
  size_t metadata_size = header.data_offset - sizeof(header);
  void *metadata = vmalloc(metadata_size, VM_RDWR);
  ASSERT(metadata_size <= UINT32_MAX);
  if ((res = d_read_n(device, sizeof(header), metadata, metadata_size)) < 0 || res != metadata_size) {
    vfree(metadata);
    if (res >= 0)
      res = -EIO;
  ...
  path_t dirpath = path_new("/", 1);
  ramfs_node_t *dir_node = mount->root;
  initrd_entry_v2_t *entry = (initrd_entry_v2_t *) metadata;
  for (int i = 0; i < header.entry_count; i++) {
    path_t path = path_new(entry->path, entry->path_len);
    path = path_strip_trailing(path, '/');
    ...
    if (entry->entry_type == 'f') {
      DPRINTF("   file  {:path} (%u bytes)\n", &path, entry->data_size);
      initrd_node_t *rd_node = kmallocz(sizeof(initrd_node_t));
      rd_node->data_offset = header.data_offset + entry->data_offset;  // convert to absolute offset
      rd_node->data_size = entry->data_size;
      rd_node->checksum = entry->checksum;
      node->data = rd_node;
      node->size = entry->data_size;
      node->ops = &initrd_vnode_ops;
    } else if (entry->entry_type == 'd') {
      ...
      DPRINTF("   link  {:path}\n", &path);
      size_t len = entry->data_size - 1;
      str_t link = str_alloc_empty(len);
      tmp = kio_writeonly_from_str(link);
      if (d_nread(device, header.data_offset + entry->data_offset, len, &tmp) < 0 || tmp.size != len) {
        EPRINTF("mount: failed to read link data for {:path}\n", &path);
        str_free(&link);
        continue;
      }
    ...
    entry = offset_ptr(entry, sizeof(struct initrd_entry_v2) + entry->path_len + 1);
  }
  hash_map_free(node_map);
  vfree(metadata);
