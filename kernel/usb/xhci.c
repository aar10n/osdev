...
//   .add_transfer = xhci_device_add_transfer,
//   .start_transfer = xhci_device_start_transfer,
//   .await_event = xhci_device_await_event,
//   .read_device_descriptor = xhci_device_read_descriptor,
//
...
// }
//
// int xhci_device_add_transfer(usb_device_t *device, usb_endpoint_t *endpoint, usb_transfer_t *transfer) {
//   xhci_device_t *dev = device_to_hd(device);
...
// }
//
// int xhci_device_start_transfer(usb_device_t *device, usb_endpoint_t *endpoint) {
//   xhci_device_t *dev = device_to_hd(device);
//   xhci_endpoint_t *ep = endpoint->host_data;
//   kassert(ep->type == get_xhci_ep_type(endpoint->type, endpoint->dir));
//
...
// }
//
// int xhci_device_await_event(usb_device_t *device, usb_endpoint_t *endpoint, usb_event_t *event) {
//   xhci_device_t *dev = device_to_hd(device);
//   xhci_endpoint_t *ep = endpoint->host_data;
//   kassert(ep->type == get_xhci_ep_type(endpoint->type, endpoint->dir));
//
...
//     kprintf("xhci_device_await_event() | USB ERROR %d\n", result.compl_code);
//   }
//   return 0;
// }
//
// int xhci_device_read_descriptor(usb_device_t *device, usb_device_descriptor_t **out) {
//   xhci_controller_t *hc = device_to_hc(device);
//   xhci_device_t *dev = device_to_hd(device);
...
// }
//
// xhci_protocol_t *_xhci_alloc_protocols(xhci_controller_t *hc) {
...
// }
//
// void _xhci_debug_port_registers(xhci_controller_t *hc, xhci_port_t *port) {
//   uint8_t n = port->number - 1;
//   uint32_t portsc = read32(hc->op_base, XHCI_PORT_SC(n));
//   kprintf("  ccs: %d\n", (portsc & PORTSC_CCS) != 0);
//   kprintf("  ped: %d\n", (portsc & PORTSC_EN) != 0);
...
// }
//
// void _xhci_debug_device_context(xhci_device_t *device) {
// }
