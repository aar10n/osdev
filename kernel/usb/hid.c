...
// static inline hid_descriptor_t *get_hid_descriptor(usb_device_t *device) {
//   // device->configs
//   usb_config_descriptor_t *config = device->configs[device->dev_config];
//   usb_if_descriptor_t *interface = offset_ptr(config, config->length);
//   return offset_ptr(interface, interface->length);
...
// void *__hid_get_report_descriptor(usb_device_t *device) {
...
int hid_device_init(usb_device_t *device) {
  ...
  // void *report_desc = hid_get_report_descriptor();
  return -1;
}
